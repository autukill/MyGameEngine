...
Home 进入时循环播放自己的流式 OGG BGM。鼠标在世界按钮内完成按下与释放后播放一次 WAV 点击音，并进入 WorldMap 底部第一岛屿；取消点击不会发声。WorldMap 展示两张无缝拼接的岛屿主体、前后云层和 20 个只读关卡节点，并播放独立的流式 OGG BGM。Home 拥有固定 Camera，WorldMap 独立声明纵向拖动、惯性和边界回弹，两个 Scene 不共享 Navigation 状态。Home 中按 `ESC` 关闭，WorldMap 中按 `ESC` 返回 Home。
