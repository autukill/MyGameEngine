...
> 状态：`BubbleTa.HomeScene` 与 WorldMap 底部第一岛屿已可运行；WorldMap 已拥有五段/100 关只读目录、可见性休眠、不可变进度快照和抗拖拽误触的选择请求边界。核心泡泡玩法尚未开始，仍处于 Gate 0 行为重建阶段。
