...
- Home 与 WorldMap 都声明 `FixedVisibleHeight(720, 1280).WithMaximumVisibleSize(960, 1280)`：窗口等比缩小时仍观察相同世界范围，只降低输出像素；宽高比改变时固定 1280 世界高度并围绕 Room 中心改变横向可见宽度，最多显示完整 960 宽 Room，之后以左右留边保护世界边界。
...
- 更宽的输出：Camera 不再越过 960 世界宽度；Hosting 把较窄的 Content RenderTarget 居中呈现，左右区域为不参与按钮命中的 Pillarbox。
