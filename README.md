...
- Gameplay Authoring：直接变换、逻辑输入、时间原语、Health/Damage、Instance Ref、确定性 Clock/Random、可持久化 Replay、状态 Hash/首次分叉诊断、实例级生命周期、轻量 Behavior 与 Scene-local 强类型 Signal。
...
- Interactive Viewport：每 RenderView 可声明统一 Pointer/Pinch、Drag、Wheel、MouseEdges、Decelerate、Animate、Bounce、Snap/Zoom 与 Clamp；Resize 自动收敛，稳定更新 0 B。
- World Chunk Streaming：消费 `ViewportSnapshot` 管理 Visible/Preloaded/Retained 三层驻留、并发/单帧启动预算、取消、失败重试和唯一 lease 生命周期。
...
文档从 [docs/README.md](docs/README.md) 进入；详细进度与已知限制见 [docs/PROJECT_STATUS.md](docs/PROJECT_STATUS.md)。真实游戏候选包括已完成首个 HomeScene、仍处于 Gate 0 行为重建阶段的《天天泡泡TA / BubbleTa》[游戏档案](games/BubbleTa/README.md)、尚未立项的《驮春 / Bloomback》[概念档案](games/Bloomback/README.md)，以及已完成 30 分钟 Gate 4 工程切片、等待外部盲测的《神意难测 / The God They Made》[游戏档案](games/TheGodTheyMade/README.md)。
...
│   ├── TileWorlds/                      # 权威 LOD0、逐层 WebP LOD、Preview 与确定性 .mgworld v3
│   ├── TileWorldStreaming/              # Zoom/滞回、后台解码、Texture Lease 与两级视觉回退
...
├── BubbleTa/                            # 《天天泡泡TA》Gate 0 重建与首个真实 HomeScene
│   ├── src/                             # 可运行 Game、首页内容包与无窗口 Simulation
│   ├── modules/                         # BubbleGrid、LevelFormat 等游戏局部单功能模块
│   ├── tools/                           # 旧关卡离线迁移工具
│   ├── tests/                           # HomeScene 等游戏行为的无窗口测试
│   └── docs/                            # 项目结构、HomeScene 与旧项目边界
└── TheGodTheyMade/
