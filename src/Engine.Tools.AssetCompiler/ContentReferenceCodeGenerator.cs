...
public sealed class ContentReferenceCodeGenerator
{
    ...
    public ContentReferenceGenerationResult Generate(ContentReferenceGenerationRequest request)
    {
        ...
        GeneratedMember[] animationEvents = CreateMembers(
            ...
            nodesById.Values
                ...
                .Distinct());
        GeneratedMember[] tileSets = CreateMembers(
            "TileSet",
            nodesById.Values
                .SelectMany(node => node.Manifest.TileSets)
                .Select(tileSet => (tileSet.Name, (string?)null)));
        GeneratedMember[] tileMaps = CreateMembers(
            "TileMap",
            nodesById.Values
                .SelectMany(node => node.Manifest.TileMaps)
                .Select(tileMap => (tileMap.Name, (string?)null)));
        GeneratedMember[] tileWorlds = CreateMembers(
            "TileWorld",
            nodesById.Values
                .SelectMany(node => node.Manifest.TileWorlds)
                .Select(tileWorld => (tileWorld.Name, (string?)null)));
    ...
    private static string RenderSource(
        ...
        IReadOnlyList<GeneratedMember> tileSets,
        IReadOnlyList<GeneratedMember> tileMaps,
        IReadOnlyList<GeneratedMember> tileWorlds)
    {
        ...
        AppendReferenceContainer(
            source,
            "TileSets",
            "global::GameEngine.Features.Tilemaps.Domain.TileSetRef",
            tileSets);
        source.AppendLine();
        AppendReferenceContainer(
            source,
            "TileMaps",
            "global::GameEngine.Features.Tilemaps.Domain.TileMapRef",
            tileMaps);
        source.AppendLine();
        AppendReferenceContainer(
            source,
            "TileWorlds",
            "global::GameEngine.Features.TileWorlds.Domain.TileWorldRef",
            tileWorlds);
        source.AppendLine("}");
