...
项目处于 Phase 1.x：最小引擎闭环已经可运行，正在从技术 Demo 向可扩展运行时收口。当前共 26 个 .NET 项目、112 个 C# 文件，Feature 拆分为 Camera、ContentAssets、RenderPipeline、SceneSystem、Sprites、StencilMasking、TextureAssets、TextureAtlas 八个独立模块。
...
- VisualTests 仍依赖人工观察，没有自动 GPU 像素回归基线。
- SceneAggregate 的 ToList、LINQ 过滤和排序仍会产生每帧分配。

## 下一里程碑：自动 GPU 回归

1. 为 VisualTests 增加固定窗口、固定时间步和确定性帧入口。
2. 增加离屏像素读取、PNG 基线和容差比较。
3. 在无显示器环境提供软件渲染或可跳过的 GPU 能力探测。
4. 首先覆盖 Sprite 原点/旋转/翻转、Stencil owner 回收和 resize 后动态效果。
...
相关说明：[Content Assets](CONTENT_ASSETS.md)、[Texture Atlas](TEXTURE_ATLAS.md)、[`GameEngine.Content.targets`](GAMEENGINE_CONTENT_TARGETS.md)、[动态渲染效果](DYNAMIC_RENDER_EFFECTS.md)。
