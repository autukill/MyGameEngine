...
使用文档按功能切片渐进维护，入口见 [文档索引](README.md)；声明式资产包详见 [Content Assets 使用指南](CONTENT_ASSETS.md)。

## 当前阶段

项目处于 Phase 1.x：最小引擎闭环已经可运行，核心模块边界基本稳定，正在从技术 Demo 向可扩展运行时收口。

当前代码规模为 22 个 .NET 项目、96 个 C# 文件。Feature 已拆为 Camera、ContentAssets、RenderPipeline、SceneSystem、Sprites、StencilMasking、TextureAssets 七个独立 module；ContentAssets 同时依赖 TextureAssets 与 Sprites，两个底层模块保持互不依赖。
...
- 8 个无窗口冒烟项目覆盖值对象、聚合、生命周期、输入沿事件、渲染状态、Sprite 帧/几何、PNG/WebP、声明式包依赖与原子回滚。

## 仍在演进

- `ShaderRef`、`ShaderLibrary` 和通用 `ShaderProgram` 已具备，但缺少 Runner 中的自定义实例 Shader 示例与统一动态 uniform API。
- SpriteLibrary 与 ContentAssets 已建立逐帧 Texture/像素矩形边界；自动 Atlas 打包、CPU 构建产物和资产热重载尚未实现。
...
- 暂无自动 TextureAtlas、物理/Spatial Hash、音频、完整资产构建管线、编辑器与 AI Bridge 的运行时代码。
