...
- 独立 `Engine.Features.WorldStreaming` 消费 `ViewportSnapshot`，以确定性行优先顺序管理 Visible/Preloaded/Retained 三层 Chunk；异步并发、单次 Update 启动量、最大跟踪数、取消、失败重试和唯一 lease 释放均有明确边界，完全加载的稳定 Snapshot 更新保持 0 B。独立 `Engine.Features.TileWorlds` 已提供权威 LOD0 RLE/碰撞、逐 Layer LOD1+ exact 无损 WebP、版本化 `.mgworld` v2、SHA-256 Chunk 校验、Content 租约和强类型引用；下一阶段连接运行时 LOD 选择与 Loader。
...
4. 小型 TileMap 编译产物继续保留严格 JSON；大型 `tileWorlds` 已编译为版本化 `.mgworld` v2，包含精确 LOD0 Tile/碰撞、逐 Layer LOD1+ WebP 与随机读取索引。异步 Loader、运行时 LOD、地图热重载和流式驻留仍留在后续切片。
