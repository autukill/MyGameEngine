...
项目处于 Phase 1.x：最小引擎闭环已经可运行，并开始由真实游戏灰盒反向验证开发体验。当前解决方案共 78 个 .NET 项目、仓库共 79 个项目；21 个正式/基础 Feature 模块保持垂直切片，`Engine.Hosting` 作为开发者入口组合根。
...
- Interactive Viewport 第一阶段提供 `ViewportController` 与固定顺序插件链：主 Camera 或每个 Render View 可声明 Drag、鼠标锚点 Wheel、逐帧平滑、帧率无关 Decelerate、ClampZoom 和 Underflow Clamp；Resize 自动重新收敛，`ViewportSnapshot` 以 Revision 暴露可见世界边界，稳定更新保持 0 B。Pinch、多 Pointer、Bounce/Animate/Snap 和 Chunk Streaming 明确保留为后续独立阶段。
