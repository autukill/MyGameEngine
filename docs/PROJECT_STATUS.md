...
- Interactive Viewport 提供 `ViewportController` 与固定顺序插件链：Core 以 `PointerId/PointerContact` 统一 Mouse、Touch、Pen，Hosting 按 Pointer 独立捕获到最上层 Render View；主 Camera 或每个 Render View 可声明 Drag、双指 Pinch/平移、鼠标锚点 Wheel、逐帧平滑、帧率无关 Decelerate、ClampZoom 和 Underflow Clamp。Resize 自动重新收敛，`ViewportSnapshot` 以 Revision 暴露可见世界边界，稳定更新保持 0 B；Bounce/Animate/Snap 和 Chunk Streaming 保留为后续阶段。
