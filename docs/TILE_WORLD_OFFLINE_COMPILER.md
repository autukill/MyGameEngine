...
当前已经完成权威 `LOD0`、预切片 Raster `LOD0`、生成式 `LOD1+` 与运行时流式适配。AssetCompiler 既可从 TileMap、TileSet 和原始 Sprite 帧确定性烘焙，也可导入 `tile_{row}_{column}.webp` 形式的既有地图；归档中只有确实存在的 Chunk 才能通过 `Contains` 查询。`Engine.Features.TileWorldStreaming` 进一步提供 Zoom LOD、滞回、后台 WebP 解码、主线程 GPU Lease、最粗可用 LOD 和 Preview 回退图。

World、Map、Cell、Tile、Chunk、LOD 和 Preview 回退图的权威定义见
[MyGameEngine 统一术语](ENGINE_TERMINOLOGY.md)。`lodCount: 1` 表示只有 LOD0，最粗可用 LOD 与 LOD0
是同一层，不会另外生成一个粗层；Preview 回退图不是 LOD，也不计入 `lodCount`。
...
- Preview 回退图会被重新编码为 exact 无损 WebP 并作为 Fallback Surface 嵌入 `.mgworld`，运行时目录不保留松散 Preview 文件。
- Preview 回退图覆盖声明的完整 `bounds`，只承担加载期视觉连续性，不生成 Tile、碰撞或 Gameplay 数据。扁平的历史 `preview.webp` 通常绑定到最底部地表 Layer；需要透明分层语义时应逐 Layer 提供 Preview 回退图。
...
- 原始 WebP Preview 回退图同样原样嵌入归档，未就绪区域按世界坐标裁取对应 UV。
...
归档格式 v3 自描述 TileSize、Raster 尺寸、Gutter、采样方式和可选逐 Layer Preview 回退图（Fallback Surface）。Fallback Surface 描述符保存 LayerIndex、图片尺寸、编码、采样、长度和 SHA-256，Payload 位于 Chunk Payload 之前并按 LayerIndex 确定性排列。索引显式区分 `AuthoritativeTiles` 与 `RasterLayers`：权威 Tile 只允许出现在 LOD0；Raster 可以从 LOD0 开始并继续覆盖 LOD1+。每个 Raster Chunk Payload 按 LayerIndex 保存 Encoding、内区尺寸、Gutter 和 WebP 字节；Chunk 外层继续使用 SHA-256 做完整性验证。
...
Viewport 不知道 `.mgworld`、Tile 或 GPU；归档 Reader 也不知道 Camera。`TileWorldStreamingSession` 在适配层组合这些模块，并保持最粗可用 LOD 作为分块回退。

## 当前限制与下一步

- 独立 Preview 回退图（Fallback Surface）已进入声明式清单和 `.mgworld v3`；没有声明时仍只使用最粗可用 LOD。
...
`2040×2040` 的 `preview.webp`。旧运行时让 Preview 回退图铺满整个世界并始终位于底层；详细 Chunk
按 Viewport 可见范围加载后覆盖其对应区域，离开保留范围后再卸载。因此加载、取消、失败或快速
移动期间都不会出现空白世界。

这个结构应被解释为“常驻全图保底层 + 按需详细层”，而不是把 401 张图片声明成普通 Texture
并随 Content Package 一次性上传。`preview.webp` 的稳定语义是 Preview 回退图（`Fallback Surface`）：只提供视觉
...
3. **Preview 回退图（已完成）**：最粗可用 LOD 继续按需驻留；可选低清全图 Preview 回退图按 Layer 独立声明并常驻，
   当最粗可用 LOD 的 Chunk 尚未解码时按缺失世界区域裁取对应 UV，不要求 Preview 回退图来自 Tile 烘焙。
4. **既有切片导入（已完成）**：离线适配器验证行列、尺寸、缺片、路径安全与 Preview 回退图的世界范围，
...
Raster Chunk，隐藏窗口 smoke 在全图视角以 LOD3 驻留 9 个 Chunk 并保持一张 Preview 回退图。仓库内自动测试
