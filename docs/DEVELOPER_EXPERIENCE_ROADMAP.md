...
统一多 Pointer/Pinch、Bounce/Animate/Snap/SnapZoom/MouseEdges 和独立 `WorldChunkStreamer` 已完成。离线 TileWorld 第一阶段也已完成：声明式 `tileWorlds`、权威 LOD0 RLE/碰撞、确定性 `.mgworld`、Chunk 完整性验证、Content 租约和强类型引用已经闭环。下一步固定为 LOD1+ 分层无损 WebP 烘焙，再接运行时 LOD 选择与 Content lease Adapter；随后补全常驻 Preview/Fallback Surface，最后才提供既有 WebP 切片导入。历史 ZL Editor 的 `12000×12000`、400 张详细切片和一张 Preview 将作为真实验收样本，不作为公共 API 特例。Chunk、异步 IO、Texture lease 和显存预算不进入 Viewport 项目。完整用法见 [Interactive Viewport](INTERACTIVE_VIEWPORT.md)、[World Chunk Streaming](WORLD_CHUNK_STREAMING.md) 与 [TileWorld 离线切片编译器](TILE_WORLD_OFFLINE_COMPILER.md)。
...
| 当前主线 | 大世界 Authoring | Viewport、Chunk Streaming 与 TileWorld LOD0 归档已完成；依次推进 LOD1+ 分层无损 WebP、运行时 LOD/Loader、Preview 保底和既有切片导入 |
...
Transform Scene/Prefab、Text 多行/Layout Buffer、Audio 短音效/流式音乐与 Tilemap/World Authoring 第一阶段已完成。大地图开发体验已经推进到 `Interactive Viewport → World Chunk Streaming → TileWorld LOD0 离线归档`；当前下一步是 LOD1+ 分层无损 WebP 烘焙，然后依次完成运行时层级切换、Preview 保底和既有切片导入。Lighting 0/1 暂列其后。HarfBuzz、Yoga C ABI 与 RmlUi Render Spike 可以独立调研，但完整 GUI 集成不能越过输入路由、IME、资源租约和 SceneGui 状态恢复；Yoga Layout Tree 不替代世界 Transform Hierarchy。
