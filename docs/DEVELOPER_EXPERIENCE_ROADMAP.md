...
| 当前主线 | 大世界 Authoring | Viewport、Chunk Streaming、LOD0、LOD1+、运行时 LOD/Loader 和最粗层保底已完成；依次推进独立 Preview Surface 和既有切片导入 |
...
Transform Scene/Prefab、Text 多行/Layout Buffer、Audio 短音效/流式音乐与 Tilemap/World Authoring 第一阶段已完成。大地图开发体验已经推进到 `Interactive Viewport → World Chunk Streaming → TileWorld LOD0 → LOD1+ 分层 WebP → 运行时 LOD/Loader → 最粗层回退`；当前下一步是独立 Preview Surface，然后接入既有切片导入。Lighting 0/1 暂列其后。HarfBuzz、Yoga C ABI 与 RmlUi Render Spike 可以独立调研，但完整 GUI 集成不能越过输入路由、IME、资源租约和 SceneGui 状态恢复；Yoga Layout Tree 不替代世界 Transform Hierarchy。
