...
确定性模拟的基础不是读取现实时间，而是让玩法只依赖明确的 Step 序列、固定 delta、逻辑输入和受控随机流。本切片提供 `SimulationClock`、固定 PCG32 `GameplayRandom` 与一键固定 UPS 配置；输入录制、状态 Hash 和完整回放留给后续切片。
...
当前可以保证：给定相同引擎版本、固定更新配置、相同 pause/time-scale 操作、相同逻辑输入调用顺序、相同随机 seed 与状态，Clock 和 RNG 序列可复现。

当前尚不能自动保证完整游戏回放，因为还缺少：

1. 按 Tick 记录和注入逻辑 Action/Axis。
2. Gameplay 状态快照或稳定 Hash。
3. 首次分叉 Tick 的诊断报告。
4. 对所有外部 IO、线程完成顺序和平台浮点差异的约束。

后续应先实现逻辑输入 Tick 录制/回放，再增加状态 Hash；不在基础 Clock 中隐藏存档、网络同步或时间回溯系统。
