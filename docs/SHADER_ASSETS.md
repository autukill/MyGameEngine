...
shaders.json 把 Shader 文件、Material Schema 和默认参数放在同一个严格版本化清单中。它同时服务于三个阶段：

1. MSBuild/AssetCompiler 在 C# 编译前完成无 GL 的静态校验。
2. Hosting 在创建 Scene 前编译 Program、创建材质并应用默认参数。
3. Shader 热重载复用清单中的固定 Program 集合，并在候选切换前执行真实 GL Uniform 契约校验。
...
{
    Material = context.GetMaterial("game.player.default")
};
...
项目设置清单路径后，共享 Target 会在 CoreCompile 前调用 AssetCompiler：
