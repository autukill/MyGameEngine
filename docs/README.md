...
- [Audio 短音效与流式音乐](AUDIO_RUNTIME.md)：声明式 WAV/OGG、OpenAL/Silent、四 Buffer 队列、SceneAudio、抢占与资源释放。
...
- [Interactive Viewport](INTERACTIVE_VIEWPORT.md)：统一 Pointer，以及 Pinch/Drag/Wheel/MouseEdges/Decelerate/Animate/Bounce/Snap/Clamp、每 RenderView 输入路由和 Snapshot 边界。
- [World Chunk Streaming](WORLD_CHUNK_STREAMING.md)：由 ViewportSnapshot 驱动的 Visible/Preloaded/Retained 驻留、加载预算、取消、失败重试和 lease 生命周期。
