...
逻辑 RenderSurface 用纯值对象描述动态效果之间的纹理数据流。领域描述符只引用 `RenderSurfaceKey`，不会持有 `RenderTarget2D`、GL 句柄、Shader 或 Pass。
...
根 Surface 是组合根拥有的借用资源，Builder 不负责释放。必须在第一个动态效果创建前完成注册；效果活跃期间不能替换根 Surface。RenderTarget resize 保持对象身份不变，因此不需要重新注册。
    ...
    IReadOnlyDictionary<InstanceId, IRenderEffectDescriptor> owners)
{
    ...
    return new RenderEffectPlan(
        key,
        inputs: new[] { RenderSurfaceKey.SceneColor },
        outputs: new[] { RenderSurfaceKey.FromEffect(key, "color") });
}
```
...
- Surface 当前只绑定 `RenderTarget2D`，格式仍为 RGBA8。
...
- HDR 与 Tone Mapping 将在该逻辑边界之上单独实现。
