...
编译缓存保存在 `obj/<Configuration>/<TargetFramework>/CompiledAssets`。运行时文件复制到 `bin/.../AssetsCompiled` 和 Publish 的 `AssetsCompiled`；构建所有权文件 `.mygame-assets.json` 不会进入运行目录。
...
- 内容格式、Atlas 行为与安全路径边界仍以 [Content Assets](CONTENT_ASSETS.md)和 [Texture Atlas](TEXTURE_ATLAS.md)文档为准。
